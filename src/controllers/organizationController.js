...
const ACCOUNT_EMAIL_RETRYABLE_STATUSES = new Set(["pending", "failed", "skipped"])
...
const attemptAccountEmailDelivery = async (user, options = {}) => {
  const { passwordOverride, forceGenerateNewPassword = false } = options

  let passwordForEmail = typeof passwordOverride === "string" && passwordOverride.trim().length > 0
    ? passwordOverride.trim()
    : ""

  if (!passwordForEmail || forceGenerateNewPassword) {
    passwordForEmail = generateTemporaryPassword()
    user.password = await bcrypt.hash(passwordForEmail, 10)
    user.mustChangePassword = true
  }

  user.accountEmailStatus = "pending"
  user.accountEmailStatusMessage = "Account email is queued for delivery."
  ...
  try {
    ...
    user.accountEmailStatusMessage = emailResult.sent
      ...
      : (emailResult.reason || "Account email could not be delivered.")
    user.accountEmailLastAttemptAt = new Date()
    user.accountEmailSentAt = emailResult.sent ? new Date() : null
    await user.save()
  } catch (error) {
    console.error(`Failed to send account creation email to ${user.email}:`, error)

    user.accountEmailStatus = "failed"
    user.accountEmailStatusMessage = error?.message || "Account email could not be delivered."
    user.accountEmailLastAttemptAt = new Date()
    user.accountEmailSentAt = null
    await user.save().catch((saveError) => {
      console.error(`Failed to persist email status for ${user.email}:`, saveError)
    })
  }
}

const queueAccountEmailRetry = (user) => {
  if (!user?.email || !ACCOUNT_EMAIL_RETRYABLE_STATUSES.has(user.accountEmailStatus)) {
    return
  }

  void attemptAccountEmailDelivery(user, { forceGenerateNewPassword: true }).catch((error) => {
    console.error(`Retry email queue failed for ${user.email}:`, error)
  })
}
...
exports.addUser = async (req, res) => {
  try {
    ...
    const user = await User.create({
      ...
      accountEmailStatusMessage: "Account email is queued for delivery.",
      accountEmailLastAttemptAt: new Date()
    })

    // Send email asynchronously (fire-and-forget) to avoid blocking the response.
    ;(async () => {
      await attemptAccountEmailDelivery(user, { passwordOverride: temporaryPassword })
    })()

    // Return response immediately after user creation
    res.status(201).json({
      success: true,
      user: user.toJSON(),
